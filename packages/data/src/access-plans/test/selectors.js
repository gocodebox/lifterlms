...
describe( 'AccessPlans/selectors', () => {
	...
	test( 'getErrors()', () => {
		...
		const errors = getErrors( { ...defaultState, errors: [ ...mockErrors ] } );
	...
	test( 'hasErrors()', () => {
		...
		const errors = hasErrors( { ...defaultState, errors: [ ...mockErrors ] } );
	...
	test( 'getPlan()', () => {
		...
		// Exists.
		expect( getPlan( { ...defaultState, plans: { ...mockPlans } }, 123 ) ).toEqual( mockPlans[ 123 ] );
