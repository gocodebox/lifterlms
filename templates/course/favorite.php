...
<div class="llms-favorite-wrapper">
	...
	<?php } else { ?>
		...
		<?php if ( $is_favorite ) : ?>
			...
			<button aria-label="<?php esc_attr_e( sprintf( __( 'Remove favorite for lesson %s', 'lifterlms' ), get_the_title( $lesson->get( 'id' ) ) ), 'lifterlms' ); ?>" data-action="unfavorite" aria-pressed="true" data-type="lesson" data-id="<?php echo esc_attr( $lesson->get( 'id' ) ); ?>" class="llms-unfavorite-btn">
				<i class="fa fa-heart llms-heart-btn"></i>
			</button>

		<?php else : ?>

			<?php /* Translators: %s: lesson title */ ?>
			<button aria-label="<?php esc_attr_e( sprintf( __( 'Add favorite for lesson %s', 'lifterlms' ), get_the_title( $lesson->get( 'id' ) ) ), 'lifterlms' ); ?>" data-action="favorite" aria-pressed="false" data-type="lesson" data-id="<?php echo esc_attr( $lesson->get( 'id' ) ); ?>" class="llms-favorite-btn">
				<i class="fa fa-heart-o llms-heart-btn"></i>
			</button>
