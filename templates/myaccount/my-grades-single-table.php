	...
	<tbody>
		<tr class="llms-section">
			<th class="llms-section_title" colspan="2">
				<?php
				// translators: %1$d: section order, %2$s: section title.
				echo esc_html( sprintf( __( 'Section %1$d: %2$s', 'lifterlms' ), $section->get( 'order' ), $section->get( 'title' ) ) );
				?>
			</th>
		...
		foreach ( $section->get_lessons() as $lesson ) :
			...
			<tr>
				<td class="llms-lesson_title" colspan="2">
					<?php
					// translators: %1$d: lesson order, %2$s: lesson title.
					echo wp_kses_post( sprintf( __( 'Lesson %1$d: %2$s', 'lifterlms' ), $lesson->get( 'order' ), $title ) );
					?>
