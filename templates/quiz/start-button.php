		...
		<input id="llms-user" name="llms-user_id" type="hidden" value="<?php echo $user_id; ?>"/>
	 	...
	 	if ($lesson->get_next_lesson() && $quiz->is_passing_score( $user_id )) {
			$t = $lesson->get_next_lesson();
	 		?>
	 		<a href="<?php echo get_permalink( $lesson->get_next_lesson() );?>" class="button llms_next_lesson" style="text-decoration:none; display:inline;"><?php _e( 'Next Lesson','lifterlms' ); ?></a>
	 		<?php
	 	}
	 	?>
