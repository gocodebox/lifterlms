...
To install tests locally you'll first need a local MySQL server (5.6 or later) and PHP 7.4+. Xdebug is required to generate code coverage reports.
...
Tests are run automatically on commits and pull requests via [GitHub Actions](https://github.com/gocodebox/lifterlms/actions/workflows/test-phpunit.yml).
...
+ Focus PHPUnit coverage on critical components — enrollment, checkout, access plans, student data, and core model methods. For large user-facing workflows, prefer E2E tests (see `tests/e2e/README.md`) over trying to unit-test every edge case.
...
+ The test suite uses the `lifterlms-tests` library which is aimed to provide shared utilities for testing the LifterLMS core, as well as LifterLMS add-ons. Many methods and utilities are available and documented in the library's GitHub repo: https://github.com/gocodebox/lifterlms-tests
