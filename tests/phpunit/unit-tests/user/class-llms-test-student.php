...
class LLMS_Test_Student extends LLMS_UnitTestCase {
	...
	public function test_get_certificates() {
		...
		$earned = $this->earn_certificate( $student->get( 'id' ), $this->create_certificate_template(), $this->factory->post->create() );

		$certs = $student->get_certificates( array() );
		$this->assertInstanceOf( 'LLMS_Awards_Query', $certs );

		$cert = $certs->get_results()[0];
		$this->assertEquals( $earned[1], $cert->get( 'id' ) );
		$this->assertEquals( $student->get( 'id' ), $cert->get_user_id() );

	}

	/**
	 * Test get_certificates() deprecated method signature.
	 *
	 * @since [version]
	 *
	...
	public function test_get_certificates_deprecated() {
		...
		$this->assertEquals( array(), $student->get_certificates() );
		...
		while ( $i < 5 ) {
			...
			$earned = $this->earn_certificate( $student->get( 'id' ), $this->create_certificate_template(), $related );

			$cert                 = new stdClass();
			$cert->post_id        = $related;
			$cert->certificate_id = $earned[1];
			$cert->earned_date    = $date;
		...
		$sort_opts = array(
			...
			'certificate_id' => 'certificate_id', // Earned cert ID.
			'meta_value'     => 'certificate_id', // actual meta value name.
		);
		...
		foreach ( $sort_opts as $input_sort => $expect_sort ) {
			...
			$this->assertEquals(
				$expected,
				$student->get_certificates( $input_sort, 'DESC', 'obj' )
			);
			$this->assertEquals(
				array_map( 'llms_get_certificate', wp_list_pluck( $expected, 'certificate_id' ) ),
				$student->get_certificates( $input_sort, 'DESC', 'certificates' )
			);
			...
			$this->assertEquals(
				$expected,
				$student->get_certificates( $input_sort, 'ASC', 'obj' )
			);
			$this->assertEquals(
				array_map( 'llms_get_certificate', wp_list_pluck( $expected, 'certificate_id' ) ),
				$student->get_certificates( $input_sort, 'ASC', 'certificates' )
			);
