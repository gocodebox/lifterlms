...
class LLMS_Test_LLMS_Access_Plan extends LLMS_PostModelUnitTestCase {
	...
	/**
	 * Test can_gateway_process() with an invalid gateway.
	 *
	 * @since [version]
	 *
	 * @return void
	 */
	public function test_can_gateway_process_invalid_gateway() {

		$this->assertFalse( $this->obj->can_gateway_process( 'fake' ) );
		$this->assertWPErrorCodeEquals( 'llms-plan-can-gateway-process-invalid', $this->obj->can_gateway_process( 'fake', true ) );

	}

	/**
	 * Test can_gateway_process().
	 *
	 * @since [version]
	 *
	 * @return void
	 */
	public function test_can_gateway_process() {
		...
		$orig_supports       = $inputs[0]->supports;
		$inputs[0]->supports = array(
			'single_payments'    => false,
			'recurring_payments' => false,
		);

		// Disabled gateway.
		$inputs[0]->set_option( 'enabled', 'no' );
		foreach ( $inputs as $input ) {
			$this->assertFalse( $this->obj->can_gateway_process( $input ) );
			$this->assertWPErrorCodeEquals( 'llms-plan-can-gateway-process-not-enabled', $this->obj->can_gateway_process( $input, true ) );
		}

		// Enable the gateway.
		$inputs[0]->set_option( 'enabled', 'yes' );

		foreach ( $inputs as $input ) {
			// Recurring plan without support.
			$this->obj->set( 'frequency', 1 );
			$this->assertFalse( $this->obj->can_gateway_process( $input ) );
			$this->assertWPErrorCodeEquals( 'llms-plan-can-gateway-process-support-recurring', $this->obj->can_gateway_process( $input, true ) );
			
			// One-time plan without support.
			$this->obj->set( 'frequency', 0 );
			$this->assertFalse( $this->obj->can_gateway_process( $input ) );
			$this->assertWPErrorCodeEquals( 'llms-plan-can-gateway-process-support-single', $this->obj->can_gateway_process( $input, true ) );
		}
		
		// Reset support.
		$inputs[0]->supports = $orig_supports;

		foreach ( $inputs as $input ) {
			
			// Recurring okay.
			$this->obj->set( 'frequency', 1 );
			$this->assertTrue( $this->obj->can_gateway_process( $input ) );
