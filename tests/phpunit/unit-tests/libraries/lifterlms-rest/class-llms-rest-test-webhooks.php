...
class LLMS_REST_Test_Webhooks extends LLMS_REST_Unit_Test_Case_Base {
	...
	public function test_create_bad_url() {
		...
		// Mock an unreachable host so the test doesn't depend on DNS resolution behavior.
		$this->mock_http_request(
			'https://sdlkfj.lifterlms.com',
			new WP_Error( 'http_request_failed', 'cURL error 6: Could not resolve host: sdlkfj.lifterlms.com' ),
			true
		);
