...
<script type="text/html" id="tmpl-llms-question-template">
	...
	<header class="llms-builder-header">
		...
		<# if ( data.get( 'question_type' ).get( 'points' ) ) { #>
			<div class="llms-question-points tip--top-left" data-tip="{{{ data.get_points_percentage() }}}">
				<input class="llms-input" min="0" max="99" name="question_points" type="number" value="{{{ data.get( 'points' ) }}}" tabindex="-1"><small><?php _e( 'points', 'lifterlms' ); ?></small>
			</div>
		<# } #>

	</header>

	<section class="llms-question-content">
		...
		<# if ( 'yes' === data.get( 'video_enabled' ) ) { #>
			<div class="llms-editable-video">
				...
				<span class="tip--top-right" data-tip="<?php esc_attr_e( 'Use YouTube, Vimeo, or Wistia video URLS.', 'lifterlms' ); ?>"><i class="fa fa-question-circle" aria-hidden="true"></i></span>
			</div>
		<# } #>
