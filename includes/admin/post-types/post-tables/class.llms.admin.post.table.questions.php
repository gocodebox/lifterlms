...
class LLMS_Admin_Post_Table_Questions {
			...
			case 'lesson' :
						...
						$edit_link = get_edit_post_link( $this->lesson_p_id );	
							if ( ! empty( $this->lesson_p_id ) ) {
								printf( __( '<a href="%1$s">%2$s</a>' ), $edit_link, get_the_title( $this->lesson_p_id ) );
							}
			...
			case 'quiz' :
					$edit_link = get_edit_post_link( $this->quiz_p_id );
						...
						if ( ! empty( $this->quiz_p_id ) ) {
							printf( __( '<a href="%1$s">%2$s</a>' ), $edit_link, get_the_title( $this->quiz_p_id ) );
						}
	...
	public function query_posts_filter( $query ){
		...
		if(!empty($quiz_ids)){	
			...
			if($l_id){
				//echo "show question of ids:".$l_id;
				
				//set query var these quizes will show 
				//$query->query_vars['post__in'] = array($l_id);
				$query->query_vars['post__in'] = $questions_ids[0];
				//$query->query_vars['post__in'] = array(9781,9782,9775,9784,9783,9778,9774,9777,9780,9776,9779);
