	...
	public function save( $post_id ) {
		if ( isset( $_POST['llms_update_enrollment_status'] ) && isset( $_POST['llms_student_old_enrollment_status'] ) && isset( $_POST['llms_student_new_enrollment_status'] ) ) {
			...
			if ( $old_status !== $new_status ) {
				...
				$order->add_note( sprintf( __( 'Student enrollment status changed from %1$s to %1$s', 'lifterlms' ), llms_get_enrollment_status_name( $old_status ), llms_get_enrollment_status_name( $new_status ) ), true );
