...
class LLMS_Analytics_Enrollments_Widget extends LLMS_Analytics_Widget {
	...
	public function set_query() {
		...
		$this->query = 'SELECT ID FROM wp_posts WHERE post_type = %s';

		$this->query = "SELECT COUNT(meta_id) FROM {$wpdb->prefix}lifterlms_user_postmeta WHERE meta_key = '_status' AND meta_value = 'Enrolled' AND updated_date BETWEEN CAST( %s AS DATE ) AND CAST( %s AS  DATE );";
