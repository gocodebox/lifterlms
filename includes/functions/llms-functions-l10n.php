...
/**
 * Load JSON format localization files for a script
 *
 * This function localizes using the WP Core's default language file directories as a after
 * checking in the LifterLMS-defined "safe" directory.
 *
 ...
 * @since [version]
 *
 * @param string      $handle       Handle for the script. The script must be registered with `wp_register_script()` for this to work!
 * @param string      $domain       Textdomain being loaded.
 * @param string|null $plugin_dir   Full path to the plugin directory, if none supplied `LLMS_PLUGIN_DIR` is used.
 * @param string|null $language_dir Relative path to the language directory within the plugin. If none supplied, `languages` is used.
 * @return void
 */
function llms_set_script_translations( $handle, $domain, $plugin_dir = null, $language_dir = null ) {

	$plugin_dir   = $plugin_dir ? $plugin_dir : LLMS_PLUGIN_DIR;
	$language_dir = $language_dir ? $language_dir : 'languages';

	$safe_dir = llms_l10n_get_safe_directory();

	// Check if the file exists in the safe directory (#1) and load it from there if it does, otherwise fall back to the default directories (#2 then #3).
	$path = load_script_textdomain( $handle, $domain, $safe_dir ) ? $safe_dir : sprintf( '%1$s/%2$s', $plugin_dir, $language_dir );
