 ...
 *
 * Manages individual lesson time tracking sessions stored in the
...
class LLMS_Lesson_Time_Session extends LLMS_Abstract_Database_Store {
	 ...
	 * @return void
	 */
	public static function expire_open_sessions( $user_id ) {
		...
		foreach ( $open_sessions as $row ) {
			...
			self::update_cached_time( $user_id, $row->lesson_id );
		}
	}
	 ...
	 */
	public static function start_session( $user_id, $lesson_id ) {

		self::expire_open_sessions( $user_id );
	 ...
	 */
	public static function get_total_seconds( $user_id, $lesson_id, $use_cache = true ) {
	 ...
	 * @return void
	 */
	public static function update_cached_time( $user_id, $lesson_id ) {

		$total = self::get_total_seconds( $user_id, $lesson_id, false );

		$lesson = llms_get_post( $lesson_id );
		if ( $lesson && is_a( $lesson, 'LLMS_Lesson' ) ) {
			$course_id = $lesson->get( 'parent_course' );
			if ( $course_id ) {
				self::update_cached_course_time( $user_id, $course_id );
			}
		}
	}
	 ...
	 */
	public static function update_cached_course_time( $user_id, $course_id ) {
		...
		foreach ( $lessons as $lid ) {
			$total += self::get_total_seconds( $user_id, $lid );
		}
	 ...
	 * @return int Total seconds.
	 */
	public static function get_course_time( $user_id, $course_id, $use_cache = true ) {
		...
		return self::update_cached_course_time( $user_id, $course_id );
	}
	 ...
	 * @return void
	 */
	public static function record_admin_override( $user_id, $lesson_id, $trigger ) {

		$lesson   = llms_get_post( $lesson_id );
		$total    = self::get_total_seconds( $user_id, $lesson_id );
	 ...
	 */
	public static function get_admin_override( $user_id, $lesson_id ) {
