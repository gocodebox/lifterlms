...
class LLMS_Form_Field {
	...
	protected function prepare() {
		...
		if ( 'html' === $this->settings['type'] && in_array( 'llms-password-strength-meter', $this->settings['classes'], true ) ) {
	...
	protected function prepare_password_strength_meter() {
		...
		if ( isset( $this->settings['min_length'] ) ) {
			...
			$meter_settings['min_length'] = $this->settings['min_length'] < 6 ? 6 : $this->settings['min_length'];

			$find[]    = '{min_length}';
			$replace[] = $this->settings['min_length'];
